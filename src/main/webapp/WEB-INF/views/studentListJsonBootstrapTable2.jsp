				 ...
				 {field: 'hetongjiner',title: '合同总金额（万元）',align: 'center', sortable: true,
					 ...
					 footerFormatter: function (value) { 
				        ...
				        for (var i in value) { 
				        	...
				        	count += value[i].hetongjiner*Math.pow(10,6); 
				        	
				        } 
				        return "合同总额："+count/Math.pow(10,6);
				    }
				},
					...
					formatter:function(value,row,index){ 
					 ...
					 footerFormatter: function (value) { var count = 0;
				        for (var i in value) { 
				        	...
				        	count += Number(value[i].kaipjiner)*Math.pow(10,6); 
				        } 
				        return "开票总额："+count/Math.pow(10,6);
				    }},
						 ...
						 footerFormatter: function (value) { var count = 0;
					        for (var i in value) { 
					        	if(Number(value[i].yifujiner)=="NaN")
					        		value[i].yifujiner=0;
					        	count += Number(value[i].yifujiner)*Math.pow(10,6);
					        } 
					        return "已付总额："+count/Math.pow(10,6);
					    }
				    },
						 ...
						 footerFormatter: function (value) { var count = 0;
					        for (var i in value) { 
					        	if(Number(value[i].yuer)=="NaN")
					        		value[i].yuer=0;					        	
					        	count += Number(value[i].yuer)*Math.pow(10,6);
					        } 
					        return "余额总额："+count/Math.pow(10,6);
					    }},
...
window.operateEvents1 = {
	...
	function operateFormatter(value, row, index) {
		return [
			...
			'<button id="btn_detail1" type="button" class="RoleOfdelete btn btn-danger  btn-sm">删除</button>',
		].join('');
	}
...
function _update_stu_form(){
	...
	if(!isNumber($("#stu_update_form input[name=shuliang]").val()))
	{alert("数量必须为数字"); 
	return  }
	if(!isNumber($("#stu_update_form input[name=danjia]").val()))
	{alert("单价必须为数字"); 
	return  }
	if(!isNumber($("#stu_update_form input[name=yifujiner]").val()))
	{alert("已付金额必须为数字"); 
	return  }
	if(!isNumber($("#stu_update_form input[name=kaipjiner]").val()))
	{alert("开票金额必须为数字"); 
	return  }
	$.ajax({
...
function _stu_add_form(){
	...
	{alert("项目名称不能为空"); 
	return  }		
	if(!isNumber($("#stu_insert_form input[name=shuliang]").val()))
	{alert("数量必须为数字"); 
	return  }
	if(!isNumber($("#stu_insert_form input[name=danjia]").val()))
	{alert("单价必须为数字"); 
	return  }
	if(!isNumber($("#stu_insert_form input[name=yifujiner]").val()))
	{alert("已付金额必须为数字"); 
	return  }
	if(!isNumber($("#stu_insert_form input[name=kaipjiner]").val()))
	{alert("开票金额必须为数字"); 
	return  }

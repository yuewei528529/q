...
public class StudentController {
	...
	public ModelAndView show(@RequestParam(value = "pn", defaultValue = "1") Integer pn) {// 不传值pn的话默认为第一页，模拟点击第三页http://192.168.3.3:8080/GzMavenSSM/student/list?pn=3
		...
		// System.err.println(pageInfo.getList());//Page{count=true, pageNum=1,
		// pageSize=5, startRow=0, endRow=5, total=642, pages=129, reasonable=true,
		// pageSizeZero=false}[Student [id=1, name=高晓松0, age=26, date=Sat Sep 01
		// 08:38:21 CST 2018], Student [id=2, name=高晓松1, age=26, date=Sat Sep 01
		// 08:38:21 CST 2018], Student [id=3, name=高晓松2, age=26, date=Sat Sep 01
		// 08:38:21 CST 2018], Student [id=4, name=高晓松3, age=26, date=Sat Sep 01
		// 08:38:21 CST 2018], Student [id=5, name=高晓松4, age=26, date=Sat Sep 01
		// 08:38:21 CST 2018]]
	...
	public AjaxMsg list(@RequestParam(value = "pn", defaultValue = "1") Integer pn) {// 不传值pn的话默认为第一页，模拟点击第三页http://192.168.3.3:8080/GzMavenSSM/student/list?pn=3
		...
		// System.out.println(AjaxMsg.ajaxResultSuccess().addResultMap("pageInfoJson",
		// pageInfo));//AjaxMsg [code=200, msg=success,
		// resultMap={pageInfoJson=PageInfo{pageNum=1, pageSize=5, size=5, startRow=1,
		// endRow=5, total=5000, pages=1000, list=Page{count=true, pageNum=1,
		// pageSize=5, startRow=0, endRow=5, total=5000, pages=1000, reasonable=true,
		// pageSizeZero=false}[Student [id=1, name=习近平, age=99, date=Fri Aug 31 12:23:37
		// CST 2018], Student [id=3, name=高晓松2, age=26, date=Fri Aug 31 12:23:37 CST
		// 2018], Student [id=4, name=高晓松3, age=26, date=Fri Aug 31 12:23:37 CST 2018],
		// Student [id=5, name=高晓松4, age=26, date=Fri Aug 31 12:23:37 CST 2018], Student
		// [id=6, name=高晓松5, age=26, date=Fri Aug 31 12:23:37 CST 2018]], prePage=0,
		// nextPage=2, isFirstPage=true, isLastPage=false, hasPreviousPage=false,
		// hasNextPage=true, navigatePages=6, navigateFirstPage=1, navigateLastPage=6,
		// navigatepageNums=[1, 2, 3, 4, 5, 6]}}]
		return AjaxMsg.ajaxResultSuccess().addResultMap("pageInfoJson", pageInfo);
		/**
		 * .解释上面一句话 AjaxMsg.ajaxResultSuccess()返回一个AjaxMsg对象，等于操作了AjaxMsg ajaxMsg = new
		 * AjaxMsg（）操作。 AjaxMsg.ajaxResultSuccess().addResultMap("pageInfoJson",
		 * pageInfo);等于ajaxMsg.addResultMap("pageInfoJson",
		 * pageInfo);所以两次对同一个AjaxAMsg对象进行的操作，并且addResultMap也无需是静态方法
		 *
		 * .返回json的结果：总的是AjaxMsg对象格式，并且附带了很多分页信息，javabean格式与json格式基本一致，所以很容易相互转换 {
		 * "code": 200, "msg": "success", "resultMap": { "pageInfoJson": { "total":
		 * 5000, "list": [{ "id": 1, "name": "习近平", "age": 99, "date": 1535689417000 },
		 * { "id": 3, "name": "高晓松2", "age": 26, "date": 1535689417000 }, { "id": 4,
		 * "name": "高晓松3", "age": 26, "date": 1535689417000 }, { "id": 5, "name":
		 * "高晓松4", "age": 26, "date": 1535689417000 }, { "id": 6, "name": "高晓松5", "age":
		 * 26, "date": 1535689417000 }], "pageNum": 1, "pageSize": 5, "size": 5,
		 * "startRow": 1, "endRow": 5, "pages": 1000, "prePage": 0, "nextPage": 2,
		 * "isFirstPage": true, "isLastPage": false, "hasPreviousPage": false,
		 * "hasNextPage": true, "navigatePages": 6, "navigatepageNums": [1, 2, 3, 4, 5,
		 * 6], "navigateFirstPage": 1, "navigateLastPage": 6, "firstPage": 1,
		 * "lastPage": 6 } } }
		 */
	...
	public AjaxMsg insert(Student student, BindingResult result) {// @Valid和BindingResult result是JSR303验证学生实体数据
		...
		// }
		/**
		 * { "code": 100, "msg": "faild", "resultMap": { "errors": { "nameUniqueErr":
		 * "用户名重复", } } }
		 * {"code":100,"msg":"faild","resultMap":{"nameUniqueErr":"用户名重复"}}
		 */

		/*
		 * if(result.hasErrors()) { //2、JSR303字段检验不合格
		 * 
		 * List<FieldError> errors=result.getFieldErrors(); System.out.println(errors);
		 * for(FieldError err :errors) { System.err.println("每次循环的总错误信息："+ err);//Field
		 * error in object 'student' on field 'age': rejected value [null]; codes
		 * [NotNull.student.age,NotNull.age,NotNull.java.lang.Integer,NotNull];
		 * arguments
		 * [org.springframework.context.support.DefaultMessageSourceResolvable: codes
		 * [student.age,age]; arguments []; default message [age]]; default message
		 * [年龄必须是正整数（后端jsr303验证1）] System.out.println("错误字段:"+ err.getField());
		 * System.out.println("错误信息:"+err.getDefaultMessage());//获取错误信息用户名可以是4-
		 * 16位字母或数字组成(后端jsr303验证） mapErr.put(err.getField(), err.getDefaultMessage()); }
		 * 
		 * return AjaxMsg.ajaxResultFaild().addResultMap("errors", mapErr);
		 *//**
			 * -返回前端的json格式数据 { "code": 100, "msg": "faild", "resultMap": { "errors": {
			 * "date": "日期不能为空（后端jsr303验证1）", "name": "用户名可以是4-16位字母或数字组成(后端jsr303验证）",
			 * "age": "年龄必须是正整数（后端jsr303验证1）"， "nameUniqueErr": "用户名重复", } } }
			 * -前端获取json格式数据很容易 if(undefined!=_response.resultMap.errors.name){//定义了说明出错了
		...
		// }
		// return count > 0 ? AjaxMsg.ajaxResultSuccess().addResultMap("id", id):
		// AjaxMsg.ajaxResultFaild();
		// }
	...
	public AjaxMsg countByName(Student student) {// 传入student对象原理见自己总结《05、SpringMVC自己学习，传入参数处理》
		...
		if (!student.getName().matches(regName)) {
			...
			// 合规
			// 2、判断用户名是否重复
			// System.out.println(student);//将这个Student [id=null, name=1234, age=null,
			// date=null]对象内容传递最终给mapper
			Long count = studentService.countByName(student);
			return count > 0 ? AjaxMsg.ajaxResultSuccess().addResultMap("statusMsg", "用户名重复")
					: AjaxMsg.ajaxResultFaild();
		}
	...
	public AjaxMsg update(Student student) {
		System.out.println(student);// Student [id=1, name=习近平, age=99, date=Thu Sep 06 00:38:21 CST
									// 2018],jsp传递过来的Json数据：name=习近平1&age=99&date=2018-09-01
									// 00:38:21&id=1，springMVC根据student对象的属性自动装配
	...
	public AjaxMsg deleteBanch(String delete_id_banch) {
		...
		return studentService.deleteBanch(delete_id_banch) > 0 ? AjaxMsg.ajaxResultSuccess()
				: AjaxMsg.ajaxResultFaild();
	}
	...
	// 显示所有student+分页，需要jsp页面通过ajax获取这里分页数据，然后响应JSON类型给浏览器（支持jsp，苹果，安卓，接口等，json数据通用作为响应拓展性很强）
	// mybatis
	// pagehelp分页插件说明：https://github.com/pagehelper/Mybatis-PageHelper/blob/master/wikis/en/HowToUse.md
	...
	public Object list(// 传入参数，通过网页--》审查元素--》NetWork-》找到getJsonPageInfoBootstraptable请求--》Headers最下面有显示
			...
			@RequestParam(value = "searchBeginDate", required = false) String searchBeginDate, // 开始时间:2018-09-20
																								// 18:27:09
	...
	public Object list1() {    
		...
		kemu cc = new kemu(null, "dd", null, null, null, null, null, null, null, null, null, null, null, null, null,
